...
| UI 框架 | `ui/tokens.py`、`ui/theme.py`、`ui/components.py`、`ui/shell.py`、`ui/timeline.py` | V6 设计令牌、主题绑定、基础组件、左导航+Router、学习时间轴。 |
| UI 页面 | `ui/pages/home.py`、`ui/pages/library.py` | 今日学习首页、棋谱一级页面。 |
| 交互与存储 | `app.py`、`ui_product.py`、`style_view.py`、`game_library.py`、`project_store.py`、`config_manager.py`、`actions.py`、`invariants.py` | Tk 工作台、产品文案、窗口、文件库、项目保存与设置、操作注册与不变量。 |
| 测试辅助 | `adversarial_harness.py`、`smoke_app.py` | 对抗检测驱动、独立冒烟入口。 |
...
`analyzer/` 当前包含 48 个 `test_*.py` 测试脚本，覆盖棋盘规则、SGF、保存恢复、引擎 IPC、队列、复盘、训练（含胜率崩盘段与走势评价）、缓存、画像、错题、判分一致性、HumanSL、LLM 教练防火墙、UI 结构与交互状态。
