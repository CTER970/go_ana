...
| 交互与存储 | `app.py`、`ui_product.py`、`style_view.py`、`game_library.py`、`online_import.py`、`project_store.py`、`config_manager.py`、`actions.py`、`invariants.py` | Tk 工作台、产品文案、窗口、文件库、在线棋谱导入（URL 直链/OGS）、项目保存与设置、操作注册与不变量。 |
| 测试辅助 | `adversarial_harness.py`、`smoke_app.py`、`domain_invariants.py` | 对抗检测驱动、独立冒烟入口、学习领域不变式。 |
| 维护工具 | `self_review.py`、`doc_sync.py` | 自审查 Agent（22 条规则）、项目说明文档同步 Agent。 |
...
`analyzer/` 当前包含 52 个 `test_*.py` 测试脚本，覆盖棋盘规则、SGF、保存恢复、引擎 IPC、队列、复盘、训练（含胜率崩盘段与走势评价）、缓存、画像、错题、判分一致性、HumanSL、LLM 教练防火墙、UI 结构与交互状态。
